...
const cartReducer = (state, action) => {
    switch (action.type) {
        case "add_to_cart": {
            ...
            return cartArticle ?
                {
                    ...state, cart: state.cart.map(element => element.id === cartArticle.id ?
                        {
                            ...element, opciones: element.opciones.find(item => item.id === opcion.id) ?
                                element.opciones.map(x => x.id === opcion.id ? { ...x, amount: parseInt(x.amount) + parseInt(opcion.amount) } :
                                    x) :
